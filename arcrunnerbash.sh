...
XRSL_META=$XRSL_FOLDER"_META/current.txt";
touch $XRSL_META; XRSL_RESULTS=$XRSL_FOLDER"_RESULTS";
mkdir $XRSL_RESULTS; cd $XRSL_FOLDER; arcsub -c $CLUSTER -o "../"$XRSL_META *.xrsl;
cd ../$XRSL_RESULTS;
while sleep $TIMEOUT; do
    arcproxy -S $PROXY;
    PVAR=$(arcstat -i "../"$XRSL_META|grep 'no jobs' | wc -l);
    echo "Prepared to finish: $PVAR";
    if [ $PVAR -gt 0 ]; then
	echo "Breaking with $PVAR" && break;
    fi;
    arcget -i "../"$XRSL_META;
done
